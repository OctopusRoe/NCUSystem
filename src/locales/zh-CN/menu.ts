export default {
  'menu.home': '首页',

  // 首页菜单
  'menu.index': '系统主页',

  // 社团管理菜单
  'menu.association': '社团管理',
  'menu.association.apply': '注册申请',
  'menu.association.settings': '信息变更',
  'menu.association.member': '成员管理',
  'menu.association.recruitment': '招新管理',
  'menu.association.upgrade': '社团升级',
  'menu.association.delete': '社团注销',
  'menu.association.dynamics': '动态管理',
  'menu.association.active': '活动管理',
  'menu.association.outlay': '经费管理',
  'menu.association.outregistration': '外出登记',
  'menu.association.homepage': '社团主页',
  'menu.association.list': '社团列表',
  'menu.association.changeoffice': '换届申请',

  // 社团指导菜单
  'menu.guide': '社团指导',
  'menu.guide.studentleader': '学生负责人',
  'menu.guide.teacherleader': '指导老师',
  'menu.guide.list': '社团列表',
  'menu.guide.memberlist': '成员列表',
  'menu.guide.registerapproval': '注册审批',
  'menu.guide.uplevel': '升级审批',
  'menu.guide.logoutapproval': '注销审批',
  'menu.guide.outsideapproval': '外出审批',
  'menu.guide.outlayapproval': '费用审批',
  'menu.guide.activeapproval': '活动审批',
  'menu.guide.dynamicsapproval': '动态审批',
  'menu.guide.changeapproval': '换届审批',

  // 考核管理菜单
  'menu.examine': '考核管理',
  'menu.examine.association': '社团年审',
  'menu.examine.head': '学生负责人考核',
  'menu.examine.teacher': '指导老师考核',

  // 社团大数据菜单
  'menu.bigdata': '社团大数据',

  // 系统设置菜单
  'menu.setting': '系统设置',
  'menu.setting.base': '基本设置',
  'menu.setting.department': '部门设置',
  'menu.setting.control': '应用管理',
  'menu.setting.person': '用户管理',
  'menu.setting.userGroup': '用户组管理',
};
