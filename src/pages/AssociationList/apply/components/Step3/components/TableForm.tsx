...
const TableForm: FC<TableFormProps> = ({ value, onChange }) => {
    ...
    const saveRow = (e: React.MouseEvent | React.KeyboardEvent, key: string) => {
        ...
        setTimeout(() => {
            ...
            if (!target.name || !target.stuid || !target.politicsstatus || !target.sex || !target.college||!target.class||!target.phone) {
                message.error('请填写完整组织成员信息。');
                ...
                if (record.editable) {
                    return (
                        <Input
                            value={text}
                            autoFocus
                            onChange={(e) => handleFieldChange(e, 'stuid', record.key)}
                            onKeyPress={(e) => handleKeyPress(e, record.key)}
                            placeholder="学号"
                        />
                    );
            ...
            render: (text: string, record: TableFormDateType) => {
                if (record.editable) {
                    return (
                        <Input
                            value={text}
                            onChange={(e) => handleFieldChange(e, 'politicsstatus', record.key)}
                            onKeyPress={(e) => handleKeyPress(e, record.key)}
                            placeholder="政治面貌"
                        />
                    );
            ...
            render: (text: string, record: TableFormDateType) => {
                if (record.editable) {
                    return (
                        <Input
                            value={text}
                            onChange={(e) => handleFieldChange(e, 'sex', record.key)}
                            onKeyPress={(e) => handleKeyPress(e, record.key)}
                            placeholder="性别"
                        />
                    );
            ...
            render: (text: string, record: TableFormDateType) => {
                if (record.editable) {
                    return (
                        <Input
                            value={text}
                            onChange={(e) => handleFieldChange(e, 'college', record.key)}
                            onKeyPress={(e) => handleKeyPress(e, record.key)}
                            placeholder="学院"
                        />
                    );
            ...
            render: (text: string, record: TableFormDateType) => {
                if (record.editable) {
                    return (
                        <Input
                            value={text}
                            onChange={(e) => handleFieldChange(e, 'class', record.key)}
                            onKeyPress={(e) => handleKeyPress(e, record.key)}
                            placeholder="班级"
                        />
                    );
