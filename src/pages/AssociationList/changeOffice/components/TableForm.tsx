...
const TableForm: FC<TableFormProps> = ({ value, onChange }) => {
  ...
  const saveRow = (e: React.MouseEvent | React.KeyboardEvent, key: string) => {
    ...
    setTimeout(() => {
      ...
      if (!target.workNumber || !target.type || !target.name || !target.department || !target.termOfOffice) {
        ...
        if (record.editable) {
          return (
            <Input
              value={text}
              autoFocus
              onChange={(e) => handleFieldChange(e, 'workNumber', record.key)}
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          return (
            <Input
              value={text}
              onChange={(e) => handleFieldChange(e, 'termOfOffice', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
              placeholder="任职时间"
            />
          );
